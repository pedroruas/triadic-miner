...
class TriadicConcept:
    ...
    def compute_triadic_relevance_index(triadic_concepts, formal_context):
        """Takes the triadic_concepts and computes the Triadic Relevance Index for
        all the Triadic Concepts in the list. 
        This index is an adaptation of the paper 'Conceptual Relevance Index for
        Identifying Actionable Formal Concepts'.
        ...
        Returns:
            triadic_concepts (list): list of TriadicConcept objects
            updated with Triadic Relevance Index
        """

        def get_compatible_generators(features, generators):
            compatible_generators = []
            for gen in generators:
                generator = format_generators_to_elementary_form(
                    gen[0], gen[1])
                if set(generator).issubset(set(features)) and set(generator) != (set(features)):
                    if gen not in compatible_generators:
                        compatible_generators.append(gen)
            return compatible_generators

        def format_generators_to_elementary_form(intent, modus):
            gens = []
            for x in intent:
                for y in modus:
                    add = str(x + ' ' + y)
        ...
        for concept in tqdm(triadic_concepts):
            ...
            if concept.extent != EMPTY_SET:
                for intent_modus in zip(all_intents, all_modus):
                    ...
                    count = 0
                    Gen1 = list()
                    relevant_attribute_alpha = []
                    relevant_attribute_alpha_E = []
                    ...
                    extent_derivation = set(
                        formal_context.extension(attributes))

                    for pair in attributes:
                        pair = set([pair])
                        attributes_check = attributes - set(pair)
                        extent_derivation = set(
                            formal_context.extension(attributes_check))
                        if extent_derivation != concept.extent:
                            count += 1
                            relevant_attribute_alpha.append(pair)
                            _pair = list(pair)[0]
                            A2, A3 = str(_pair).split(' ')[
                                0], str(_pair).split(' ')[1]
                            Gen1.append([A2, A3])
                    ...
                    if count == 0:
                        for pair in attributes:
                            pair = set([pair])
                            double_prime_pair = set(formal_context.intension(
                                formal_context.extension(list(pair))))
                            pair_prime = set(
                                formal_context.extension(list(pair)))
                            if (len(double_prime_pair) > 1) and (pair_prime == concept.extent):
                                count += 1
                                relevant_attribute_alpha_E.append(pair)
                                _pair = list(pair)[0]
                                A2, A3 = str(_pair).split(' ')[
                                    0], str(_pair).split(' ')[1]
                                Gen1.append([A2, A3])
                    ...
                    compatible_generators = get_compatible_generators(
                        attributes, generators)
                    final_compatible_generators = []

                    for com_gen in compatible_generators:
                        for gen in Gen1:
                            if com_gen != gen:
                                final_compatible_generators.append(com_gen)
                    numerator = final_compatible_generators
                    if len(numerator) > 1:
                        beta = min(len(numerator) /
                                   len(compatible_generators), 1)
                    else:
                    ...
                    if not flag:
                        triadic_concepts[triadic_concepts.index(concept.extent)].triadic_relevance_index.append([
                            ...
                            {'triadic_concept': (concept.extent, intent, modus),
                             ...
                             'relevant_attribute_alpha': relevant_attribute_alpha,
                             'attribute_alpha_E': relevant_attribute_alpha_E}])
